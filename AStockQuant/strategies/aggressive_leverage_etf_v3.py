# -*- coding: utf-8 -*-
"""
Aggressive Leverage ETF Strategy (V3)
Target: 100%+ annual return, 30%+ monthly return
Features:
- Daily rebalancing, only hold TOP 1 strongest ETF
- 3x margin leverage (full leverage only in bull market)
- 6% take profit, 2% stop loss
- 2-day max hold period
- 100% cash in non-bull market
"""
import numpy as np
import pandas as pd
...
def calculate_momentum_score(df, window=3):
    """Calculate ultra short-term momentum score"""
...
def get_market_regime(date, data):
    ...
    ma5 = hs300["close"].rolling(5).mean().loc[:date].iloc[-1]
    ma10 = hs300["close"].rolling(10).mean().loc[:date].iloc[-1]
...
def backtest(start_date="2024-01-01", end_date="2026-03-31", initial_capital=1000000):
    ...
    for date in valid_dates:
        ...
        if regime == "bull":
            ...
            leverage = LEVERAGE_DOWN
        available_cash_total = equity * leverage
        ...
        for code, pos in positions.items():
            ...
            if return_pct >= TAKE_PROFIT_PCT or return_pct <= -STOP_LOSS_PCT or hold_days >= MAX_HOLD_DAYS or regime != "bull":
                sell_amount = pos["quantity"] * current_price
                available_cash_total += sell_amount
        ...
        # 非牛市行情空仓
        if regime != "bull":
            net_equity = available_cash_total - (equity * (leverage - 1))
            equity_curve.append({
                "date": date.strftime("%Y-%m-%d"),
                "equity": net_equity,
                "cash": available_cash_total,
                "position_count": 0,
                "regime": regime
            })
            equity = net_equity
        ...
        # 满仓买入最强的ETF
        if len(top_codes) and len(positions) == 0:
            code = top_codes[0]
            if date not in etf_data[code].index: continue
            current_price = etf_data[code]["open"].loc[date]
            quantity = int(available_cash_total / current_price / 100) * 100
            if quantity >= 100:
                positions[code] = {
                ...
                available_cash_total -= quantity * current_price
                trade_history.append({
                    "date": date.strftime("%Y-%m-%d"),
                    "code": code,
                    "direction": "buy",
        ...
        net_equity = (available_cash_total + position_value) - (equity * (leverage - 1))
    ...
    # 计算月度收益
    monthly_returns = df_equity["equity"].resample("ME").last().pct_change().dropna()
    ...
    output_path = REPORT_DIR / "aggressive_leverage_results_v3.json"
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    
    print("=== Aggressive Leverage ETF Strategy V3 Results ===")
